...
from the pool. *VERIFIED (restricted local SBF evidence; production source
STOP):* the local resolution route now derives and authenticates one
canonical, sealed, program-owned single-page source receipt and refuses
same-domain evidence or account substitution; that source lifecycle is
constructible only in a deliberately non-production mock-provider build, the
default build registers no provider release and fails closed before source
state or value admission, and no production source pipeline exists. I have
implemented separate host-side kernels, formal models, and
...
orders, cross pages, charge fees, or constitute a venue. A live batch path
can also initialize, freeze, and submit bounded full-width candidates in
local banks, but its measured full top-three selection reaches exactly the
1,400,000-CU transaction ceiling and rolls back every watched byte and
lamport --- a measured, non-promotable STOP --- and the successor selection
design is model and design only, with no live instruction ABI or SBF route.
Deterministic recomputation of a frozen rule is therefore the promoted
verification story here; onchain selection is not, and any future selection
claim would be limited to the best valid submitted candidate admitted before
an immutable close boundary.

Two bounded research artifacts sharpen the recomputation point with a worked
negative. *VERIFIED (bounded Clear semantics only):* an exact synthetic
energy-dispatch relation fixes three padded providers, three periods, two
buses, and integer output atoms zero through four; its 156-byte plaintext
witness and 176-byte public-result codec are deterministic. In the
representative fixture the Clear oracle examines 8,025 trajectory pairs,
finds 468 feasible complete schedules, and selects canonical objective 56. A
fully recommitted cost-60 plan is physically feasible and conserves
settlement exactly, but is rejected only because the verifier repeats the
frozen global-optimum and tie rule. Twenty-five debug and release tests and a
recorded deterministic corpus pass. This is synthetic same-implementation
evidence, not energy-market validation, an independent verifier, a proof,
privacy, or deployment. *VERIFIED (bounded encrypted candidate-validation
experiment, not an encrypted solver):* an independently re-specified
`tfhe-rs` evaluator whose evaluation API holds no client key evaluates a
caller-supplied encrypted dispatch for physical feasibility and exact
settlement conservation. Both canonical cost 56 and the feasible, conserving
cost 60 pass those predicates; a forged cost-59 settlement attached to the
cost-60 plan fails conservation. Only a separate Clear enumeration
establishes that 56 is the canonical optimum; the encrypted evaluator
performs no global search or optimality check. The same source reproduced
those predicate outcomes in separate arm64 and x86_64 CPU campaigns, and the
x86 host's detected GPUs were not used. These one-process experiments provide
no vFHE proof, evaluator-correctness proof, threshold custody, selective
result release, network privacy, private SBF settlement, or deployment, and
no part of them is described as Dark. For reporting policy the lesson is
exact: a validator that checks stated predicates can accept a feasible,
conserving, and still wrong submission; only recomputation of the complete
frozen rule verifies a selection.
...
*VERIFIED (restricted local SBF evidence; production source STOP):* the local
route now derives and authenticates one canonical, sealed, program-owned
single-page source receipt and refuses same-domain evidence or account
substitution, and the default build fails closed with no registered provider
release; but permissionless source construction exists only in a deliberately
non-production mock-provider build, and no production provider authenticator,
parser, provider-availability guarantee, multipage archive, or deployable
source pipeline exists, so the qualifying claim holds only for that
restricted local window. Reporting vocabulary should distinguish pending, disputed,
...
separate focused tests cover source-receipt-authenticated degree-one through
degree-three point resolution with internal and exact-lot bearer redemption.
These campaigns authenticate one canonical sealed local source receipt whose
underlying state is mock-constructed; they establish neither a production
source pipeline nor a complete permissionless lifecycle. Their
...
#table(
  ...
  [Selected construction, resolution, redemption, custody, staged-resolution, submission, and settlement paths execute in local SBF tests], [A genesis-assisted 22-transaction signed sequential categorical custody walk; blank-bank ordinary-wallet construction of categorical and native markets; source-receipt-authenticated native point-resolution tests with internal and exact-lot bearer redemption; one measured staged prepaid resolution route within the selected compute-headroom profile; one measured full-width candidate submission, with full top-three selection a measured 1,400,000-CU rollback STOP and the successor selection design model-only; and one restricted same-page full-fill single-claim zero-fee settlement slice. Tested subsets only; no deployment or complete venue],
