...
resolution, redemption, reservation, staged-resolution, and settlement
campaigns. The strongest
categorical custody evidence is a genesis-assisted 22-transaction signed
sequential walk;
...
evidence covers source-receipt-authenticated point resolution with internal
and exact-lot bearer redemption for degrees one through three, plus one
measured staged prepaid resolution route inside the selected compute-headroom
policy. The settlement evidence is one same-page, full-fill,
...
Permissionless source construction executes only against a deliberately
non-production stand-in provider; the default build registers no provider or
parser release and fails closed before source state or value admission. This
edition does not claim a production provider authenticator or parser,
production price history, fractional-lot bearer exit, a complete venue, or an
end-to-end permissionless lifecycle. A final public edition should cite
frozen public repository commits, exact paths, and the dated devnet
deployment record if the repositories are public by filing time; otherwise it
should retain this limited description and avoid claims of independent public
reproducibility.
