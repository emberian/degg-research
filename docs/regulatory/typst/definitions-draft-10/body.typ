...
- *Observation (restricted local evidence; production source STOP).* The
  qualifying design freezes a source specification, authenticates a complete
  price history from that named source over the stated window, and computes
  the realized band without a person choosing the value. In local SBF tests,
  resolution now derives and authenticates one canonical, sealed,
  program-owned single-page source receipt and refuses same-domain evidence
  or account substitution; a separate, deliberately non-production
  mock-provider build constructs that source lifecycle, while the default
  build registers no provider release and fails closed before source state or
  value admission. No production provider, parser, or deployable source
  pipeline exists, so the qualifying property holds only for that restricted
  local window. The rule removes one form of reporting discretion, not the
  ability of trading to influence the underlying price.
...
categorical campaign completed a 22-transaction signed, sequential custody
walk through issuance, resolution, internal and bearer redemption, and
collateral withdrawal. Further focused campaigns execute degree-one through
degree-three point resolution --- authenticated against one canonical,
sealed, program-owned single-page source receipt --- with internal and
exact-lot bearer redemption, one measured staged prepaid resolution route
inside the project's selected compute-headroom policy, and one same-page,
full-fill, single-claim, zero-fee settlement slice. These are tested subsets,
not a complete venue: the source lifecycle is constructible only in a
deliberately non-production mock-provider build while the default build fails
closed with no registered provider, nondivisible native lots refuse
redemption, the monolithic occupation-resolution route's initial admission
and full batch selection are measured compute STOPs, and no production source
adapter, general partial or portfolio settlement, or end-to-end
permissionless lifecycle exists.
...
#table(
  ...
  [Core accounting and selected construction, resolution, redemption, custody, staged-resolution, and settlement paths execute in local SBF tests], [A 22-transaction genesis-assisted signed sequential categorical custody walk; blank-bank ordinary-wallet construction of categorical and native markets; source-receipt-authenticated native point-resolution tests with internal and exact-lot bearer redemption; one measured staged prepaid resolution route within the selected compute-headroom profile; and one restricted same-page full-fill single-claim zero-fee settlement slice. Tested subsets only; no deployment, production provider, full venue, fractional-lot bearer exit, or end-to-end permissionless lifecycle],
