//! SHA-256, implemented freshly from FIPS 180-4 so the crate stays
//! dependency-free. Used only to name canonical byte strings; nothing here is
//! a hiding commitment, and no privacy property follows from it.
...
pub fn sha256(message: &[u8]) -> [u8; 32] {
    ...
    let tail_blocks = if remainder.len() + 9 <= 64 { 1 } else { 2 };
    tail[tail_blocks * 64 - 8..tail_blocks * 64].copy_from_slice(&bit_length.to_be_bytes());
