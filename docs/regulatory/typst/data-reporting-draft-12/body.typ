...
protocol, Dragon's Clutch, put it on a public ledger, and looked at what the
ledger does and does not record. This comment answers Questions 3, 8, and 19
of the joint request#note_ref(1) from that vantage, and takes no position on
the others.
...
modifications, allocation, and the meaning of a fork or dispute --- and where
several ledger instructions jointly implement one reportable event, or one
transaction contains several, the hash-to-event mapping fails in both
directions. A transaction hash is an address of bytes. What the regime needs
instead is three records, not one public event forced to serve every purpose:

1. a *public transparency record* holding the fields justified by price
   discovery and public market integrity;
2. an *exact confidential regulatory record* holding the owner-linked order,
   funding, execution, position, and settlement lifecycle needed for
   supervision, examination, and enforcement; and
3. a *machine-verifiable validation package* holding versioned schemas,
...
1. Define *normalized lifecycle events* for onchain transactions, each bound
   ...
   provenance added to --- never substituted for --- ordinary economic and
...
3. Separate the three records, and treat public dissemination as a
   documented, versioned, deterministic *leakage policy*: every public field
   justified by a stated transparency purpose and examined for
   re-identification and strategy-inference risk, with the confidential
   record exact, complete, and timely however conservative the public record
   becomes.
...
and governance paths. Whether a Dark architecture --- no general opening path
beyond a frozen leakage function and authorized local outputs --- can satisfy
every applicable reporting, correction, examination, and enforcement
objective is a question analysis cannot yet close; test it obligation by
obligation, rather than answering it with the word "private."
...
segregated pool that belongs to one market only and receives a complete claim
set --- one claim per cell of an exhaustive partition, or a smooth
...
authenticated data-source release is compiled into it, and none is; local
tests exercise the source lifecycle against a deliberately non-production
stand-in provider. At each milestone below, ask which records exist, which
are public bytes, and which exist only if somebody is required to keep them.

*Publication.* The market template is created on the ledger: partition rule,
observation-program version, batch policy, settlement terms. All of it is
public, and none of it is reportable as a transaction --- there are no
parties, no funding, and no exposure. An adapter mapping this ledger write to
"trade" would invent a trade with no participants. Ledger writes precede
reportable events.
...
beneficial identity. The separate net contingent-exposure value is zero here:
a complete set plus the unconditional right to recombine it into its
collateral is fully hedged and interchangeable with the deposit itself. Net
contingent exposure arises later, when the depositor sells some claims and
keeps others --- and that sale may occur in a venue whose individual orders
never touch this ledger. A hash-to-trade mapping that labels funding itself
as net contingent exposure records exposure that does not exist; a mapping
that ignores offchain events misses the transaction where it actually arises.
Supervision needs a confidential account-to-party linkage, so that gross
issuance and funding, the later orders, the unbalancing transaction, and
eventual redemption read as one owner's exact lifecycle.
...
clearing rule is deterministic over a frozen book, anyone authorized to hold
that book can recompute the result. I built the batch verifier to accept a
submitted clearing only if recomputation from the frozen book reproduces it
exactly, never trusting the submitter's claimed quantities. A reporting rule
with that shape is more than a format check: the rule is the check. The
onchain evidence is narrower than the recomputation principle. My local
...
ceiling on every measured row in a single-bank-profile campaign. So
deterministic recomputation of a frozen rule is the verification story here,
onchain selection is not, and any future selection claim is limited to the
...
pairs, finds 468 feasible complete schedules, and selects canonical objective
56. A fully recommitted cost-60 plan is physically feasible and conserves
settlement exactly, and is rejected only because the verifier repeats the
frozen global-optimum and tie rule. Separately, a `tfhe-rs` evaluator whose
evaluation API holds no client key checks a caller-supplied encrypted
dispatch for feasibility and exact settlement conservation: both cost 56 and
the feasible cost 60 pass those predicates, and a forged cost-59 settlement
fails conservation --- but only the Clear enumeration establishes that 56 is
optimal, because the encrypted evaluator performs no global search. Both are
synthetic, single-process experiments, not energy-market validation,
verifiable-encryption proofs, custody designs, or anything Dark. For
reporting policy the lesson is exact: a validator that checks stated
predicates can accept a feasible, conserving, and still wrong submission;
only recomputation of the complete frozen rule verifies a selection.
...
adjudicator chooses among outcomes. In my local campaigns, resolution derives
and authenticates one canonical, sealed, program-owned source receipt and
refuses substitutes --- exercised against the non-production stand-in
provider, since the default build compiles in no production source release.
Reporting vocabulary should distinguish pending, disputed, unsupported,
expired, and resolved states. Even after certification, a ledger
...
*Settlement.* The realized claims redeem from the pool. My campaigns run this
end to end --- a 22-transaction signed custody walk through issuance,
resolution, internal and bearer redemption, and full withdrawal, plus
per-degree walks for the smooth claim families --- and their public bytes show
outflows, but not which redemption closes which position of which legal or
beneficial owner at what gain. The confidential record supplies that linkage;
the public record need not.

= Question 3: report economic events, not ledger writes

The counterargument is that the ledger is already a complete, tamper-evident
record. It is neither complete nor a report, for the reasons stated at the
outset. A reporting framework for onchain markets should distinguish, at
...
accepted-set inclusion; match, execution, and allocation; pending, disputed,
unsupported, or expired resolution; certification; early exit, unwind, or
replacement; settlement, delivery, maturity, or correction; and replacement
caused by reorganization or dispute. Each report should bind the complete set
of ledger sources that implement it. Provenance fields interpret the economic
event; they do not replace economic and counterparty fields.
...
and revocation governance; a collateral proof does not establish custody or
absence of encumbrance unless those facts are inside the relation and bound
to the actual settlement. Unless the Commissions determine that a proof is
itself the required datum, the committed evidence needed to reconstruct the
event should be preserved, and the record should identify the proof system,
statement version, verifier, public-input digest, result, and the retention
location of the supporting evidence. The encrypted candidate-validation
experiment above is the concrete case: encrypted execution of stated
predicates establishes neither global optimality nor correct evaluation, so a
proof-shaped artifact satisfies a reporting element only when the required
proposition is exactly the one its statement encodes.
...
bytes, permanently. Removing a name does not remove identity. The combination
of wallet, timestamp, size, price, product, and settlement destination can
re-identify a trader, and repeated behavior can reveal entry policy,
inventory limits, hedge timing, or an algorithm's responses; sparse or
bespoke markets heighten the risk, because an otherwise anonymous event can
be unique. The current transparent offline batch prototype does not conceal
individual orders. In a proposed Shielded form a named executor or committee
could keep them from public view, but a settlement graph paying redemptions
directly to the original wallets could undo that concealment after the fact.
...
Evaluation should measure both the dissemination policy and this
independently observable leakage, stating per surface who can observe it,
when, and whether a design closes, narrows, or leaves it open. Each public
field should carry a stated transparency objective and a reviewed
...
record, which already must hold exact creation and continuation data with
counterparty and transaction identifiers,#note_ref(3) under repository
confidentiality and access rules.#note_ref(4)

The basis for my leakage statements is deliberately modest: a small
deterministic laboratory that replays four synthetic trading traces against
three transcript designs and records which fields each design mechanically
reveals and which deductions those fields enable, keeping the two categories
separate. It measures no anonymity, cryptographic leakage, timing behavior,
or real market. Its value is the discipline --- every "this design hides X"
sentence is checked against an explicit list of what the transcript contains
--- and I recommend the same for any dissemination policy: enumerate the
fields, then defend each one.

*Clear, Shielded, and Dark.* My research uses three terms exactly, because
conflating them produces bad reporting policy in both directions. *Clear:*
the specified state and computation are public. *Shielded:* a named executor,
committee, or auditor may learn private inputs. *Dark:* no actor learns
anything beyond a frozen leakage function and its own authorized local
output, within an explicit corruption model. A due-process threshold opening
path is regulator-observable Shielded, not Dark; an operator-readable
encrypted database is Shielded; unpadded ingress, observable timing, or a
public settlement graph can make a purportedly Dark design Shielded in fact.
Clear and regulator-observable Shielded systems can be evaluated against
confidential reporting obligations by inspecting their data and governance
paths. Whether a true Dark architecture can satisfy each obligation is the
open question stated at the outset; my own research has not produced an
end-to-end Dark system. One bounded TFHE experiment checks an encrypted
candidate's feasibility and exact conservation without the evaluator holding
the client key, but it is a single process with no custody, release, network,
or settlement design, and no composed Clear, Shielded, or Dark venue exists.

= Question 19: governed machine-readable rules

The Commissions have already received the general form of the right answer.
The comment of Ariadne Dataworks Ltd. on this docket recommends publishing
the reporting framework itself as structured data: a joint machine-readable
...
normative lifecycle event model with worked examples.#note_ref(5) I endorse
those three recommendations rather than restating them; adopted with the
governance that comment describes, they would carry most of what a
machine-readable regime needs. This comment's space goes to three layers the
proposal does not cover.

*Bind the validators to the three-record structure.* Each of the three
records needs its own schema and validators, and the validation package is
itself the third record --- the one that can be fully public at no
confidentiality cost, because it contains no transaction data at all:
schemas, deterministic predicates, conformance vectors, and provenance
bindings, publishable in their entirety even where everything they validate
is confidential. Publishing it buys independent implementations that agree on
what the rules accept.

*Where a rule is deterministic over committed inputs, publish the validator
in recomputation form.* A conformance corpus establishes only that an
implementation agrees with the published examples. A recomputation validator
is stronger: it re-derives the reported answer from the committed inputs and
accepts only exact agreement --- the rule is the check. The worked example exhibited the strongest form, a
clearing rule deterministic over a frozen book, checkable by anyone through
full recomputation; where a reporting rule can have that shape, disagreement
about what happened reduces to disagreement about inputs, exactly where a
regulator wants it. Correction authority can be equally concrete: in my
formal models I have machine-checked that a correction's authorized actor,
target record, permitted fields, and governing rule version can be fixed in
advance and enforced mechanically --- "who may correct this report, to what,
under which version" can be a typed object rather than a convention. A
certification should be required before a pending outcome becomes the final
regulatory state, and rejected or superseded transitions should remain
auditable without being counted as trades.
...
approximate it. "The rule rejects this" and "the backend cannot answer this"
are different facts, and a validator that conflates them will misreport both:
a rejection stream that silently absorbs outages understates data quality
exactly when the system is least healthy, and the converse hides violations
behind operational noise. A reporting party should not satisfy an obligation
by submitting into an outage, and an examiner should be able to tell, from
the record alone, which of the two happened.
