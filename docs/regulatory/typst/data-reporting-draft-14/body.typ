...
protocol, Dragon's Clutch, ran it on a real validator, and studied what it
...
run in local test banks and, at the strongest, on a real loopback validator:
a 44-transaction signed, confirmed walk dated August 20, 2026 drives the
complete general clearing lifecycle, policy artifact through portfolio
full-pair settlement, with 38 watched accounts byte-reloaded per step,
conservation re-derived from the observed bytes, and a deliberately
corrupted expectation failing red as required, against a sealed build with
all 100 declared manifest gates executed and an independent portable
attestation of 44 of 44. No public cluster runs them; a devnet deployment is
authorized but unfunded. The default build refuses every deposit until an
authenticated data-source release is compiled in (none is); local tests run
the source lifecycle against a deliberately non-production stand-in
provider. At each milestone: which records exist, which are public bytes,
and which exist only if somebody must keep them?
...
check. The onchain evidence now closes end to end: portfolio books place,
freeze, walk to verified through a streaming relation whose onchain verdict
is byte-equal to the host relation's, select among retained candidates by
...
the signed validator walk above, unpromoted, fees forced zero, partial fills
still open. Full top-three selection once consumed exactly the
1,400,000-compute-unit transaction ceiling and rolled back --- a measured
stop whose cause proved to be a software SHA-256 implementation, not the
verification design; with the runtime's hashing syscall it completes and
commits at 226,071 compute units. Onchain selection is not the verification
story; a selection claim covers only the best valid submitted candidate
...
verifiable-encryption proof, or custody design. A
...
internal and bearer redemption, and full withdrawal; per-degree walks for the
smooth claim families; and the validator walk's portfolio settlement.
Terminal close now completes the record: a finished epoch's machinery closes
in dependency order, exact recorded principal returning to each recorded
payer through a funding ledger --- a cleared epoch reclaims 531,639,600 of
its 531,652,377 lamports, the residual one declared-permanent 64-byte policy
artifact, in laboratory bank evidence --- so who funded which account, and
who was repaid, is itself a kept, checkable record. Public bytes show
outflows, not which redemption closes which position of which legal or
beneficial owner at what gain --- the confidential record's linkage, not the
public record's.
...
#claim_table(
  ...
  [Exact categorical and degree-one through degree-three B-spline payout semantics], [Exact-rational Rust kernels with canonical largest-remainder quantization; 212 zero-sorry Lean theorems for the named construction properties; no whole-implementation refinement proof],
  [Local SBF and loopback-validator execution of construction, resolution, redemption, custody, staged resolution, general clearing, and portfolio settlement], [Signed and bank campaigns: the 22-transaction custody walk, per-degree blank-bank walks, the general-plane bank suites, and the 44-transaction signed validator walk of August 20, 2026 against the sealed build (100/100 executed manifest gates; independent portable attestation 44/44); full top-three selection completing at 226,071 CU after a hasher fix dissolved its measured 1,400,000-CU rollback stop],
  ...
  [No public-cluster deployment (devnet authorized but unfunded); valueless test units; deposits refused by default], [Repository status and decision records dated August 20, 2026],
)
