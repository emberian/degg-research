...
I am an independent software and formal-methods researcher. The Committee
held its inaugural meeting on August 20, 2026; this statement is a written
contribution to that meeting's record, within the notice's August 27
window.#note_ref(9) I built a staged claim market, Dragon's Clutch, ran it,
and measured where it stops. This statement reports the results, responds to
the meeting, and asks the Committee to recommend six work products.

The Committee met against its published agenda, and Chairman Selig's opening
remarks --- posted before the meeting convened --- announce a "Roadmap for
the New Frontier of Finance" whose three tracks (crypto capital markets,
compute markets, prediction markets) each touch this record, along with
expected amendments to parts 38 and 40 for contract markets listing event
contracts and a direction to staff to "engage with developers of onchain
finance protocols to establish ways in which developers can offer their
protocols in a legal and compliant manner."#note_ref(13) This statement and
its two companion comments on the joint definitions and data-reporting
dockets are that engagement from one developer, in writing. The meeting's
discussion reached formal verification, and that is the ground this
statement serves best: proofs read as evidence of exactly the propositions
their statements encode, machine-checked negatives an examiner can probe
without trusting the builder, and a model-to-implementation correspondence
that is today reviewed and disclosed rather than proved --- the discipline
Positions 5 through 7 and the negatives table turn into recommendations.

One track is already concrete: the compute derivatives request for comment,
issued the day before the meeting and named in the opening remarks as the
Roadmap's first compute step.#note_ref(14) Its questions concentrate on
settlement-reference integrity --- whether an index computed from data the
Commission may not be able to observe, verify, or surveil can anchor a
contract, and how to weigh parties best positioned to influence a reference
price who also supply the data it is computed from. Positions 7 and 8 treat
those questions in miniature, and the clearing architecture reported here
--- frozen reference specifications, recomputation-verified selection, exact
conservation --- is one worked answer shape; I intend to comment in that
proceeding's window when it opens. Same architecture, different underlier;
the observations here are about the clearing layer, not compute cash
markets.
...
2026 drives the complete general clearing lifecycle --- policy artifact
through portfolio full-pair settlement --- with 38 watched accounts
byte-reloaded at every step, conservation re-derived from the observed
bytes, and a deliberately corrupted expectation failing red as required,
against a sealed build whose manifest executed all 100 declared gates and
which a second machine independently attested at 44 of 44 portable gates.
No public cluster runs them; a devnet deployment is authorized but unfunded.
Test units are valueless, and the default build refuses every deposit until
an authenticated data-source release is compiled in; none is. The appendix
gives every claim's basis.
...
and profile accounts; and creates categorical or native markets. Predictable
program-state and token accounts tolerate honest over-rent prefunding; a
byte-bearing late target refuses atomically. The general epoch, order-page,
candidate, pot, and receipt plane now constructs and runs; not yet: a
complete permissionless lifecycle.
...
changes); and the general clearing plane --- portfolio books place, freeze,
walk to verified through a streaming relation whose onchain verdict is
byte-equal to the host relation's, select among retained candidates by
re-derived tie digests, entitle per-slice receipts, and settle a portfolio
full pair with exact whole-plane conservation --- in laboratory banks and
the signed validator walk, unpromoted, fees forced zero. Terminal close
paths now end the story: a finished epoch's machinery closes in dependency
order, returning exact recorded principal to each recorded payer --- a
cleared epoch reclaims 531,639,600 of its 531,652,377 lamports, the residual
one declared-permanent 64-byte policy artifact --- in laboratory bank
evidence. Not yet: partial fills, fee-bearing settlement, or a production
source pipeline.
...
extrapolated-shape, or transaction-inclusion evidence.
...
cause-level change and re-measurement tell those apart.
...
not answered; Dark is a long-horizon research boundary.
...
discipline: 212 Lean theorems --- zero sorry, zero project axioms, Lean
adopted as the substrate of record --- establish model-level complete-set,
solvency, guarded-transition, and B-spline construction and quantization
properties; one pinned Verus run checks a single internal-transfer arithmetic
seam; none is refined to the complete Rust or SBF runtime. A proof
...
reference market; it does not remove it.* The agenda named market
...
assumptions, not a measurement of any real venue. The ask is for the
*inputs* --- require the reference
...
The meeting's agenda included an agentic-finance session.#note_ref(12)
...
enumerated trusted roles; no operatorless agent exists. The market artifacts
are no further along: an ordinary wallet can
...
The components do not presently compose into a production, permissionless,
source-authenticated, end-to-end market system of any privacy modality.
Machine-checked properties are properties of models, and no refinement proof
connects them to whole-protocol runtime behavior. The Committee's duties are
solely advisory,#note_ref(8) and I ask only that it recommend work.
...
#claim_table(
  ...
  [The meeting recitals (the Roadmap tracks, the parts 38 and 40 expectation, the developer-engagement direction) and the compute request's question areas], [The Chairman's posted opening remarks and the issued request for comment; source notes 13 and 14],
  ...
  [The staged accounting; machine-checked B-spline complete-set and solvency properties], [Exact Rust kernels with oracle tests; 212 zero-sorry Lean theorems for the named model properties, Lean the adopted substrate of record; one Verus-checked arithmetic seam; no whole-implementation refinement proof],
  [Milestone execution (construction, custody, point-resolution, redemption, staged resolution, general clearing, portfolio settlement) in local SBF banks and on a loopback validator], [Local signed and bank campaigns: the 22-transaction custody walk, per-degree blank-bank walks, a five-candidate staged-clearing campaign, the general-plane bank suites, and the 44-transaction signed validator walk of August 20, 2026, against the sealed build (100/100 executed manifest gates; independent portable attestation 44/44); the measured compute boundaries above],
  [Terminal close: exact recorded principal to recorded payers; a cleared epoch reclaims 531,639,600 of 531,652,377 lamports, residual one declared-permanent 64-byte policy artifact], [Real-SBF bank walk with hostile close refusals and lamport-exact conservation gates; repository merge record dated August 20, 2026; laboratory evidence, unpromoted],
  ...
  [The manipulation-cost observation], [Offline deterministic exact-integer experiment over synthetic constant-product pools, cross-checked independently; a lower bound only under its stated recovery model; no market data, no number in this statement],
  ...
  [The deployment characterization: no public-cluster deployment; devnet authorized but unfunded; valueless test units throughout], [Repository status and decision records dated August 20, 2026],
)
