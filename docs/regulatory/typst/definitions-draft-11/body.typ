...
claim protocol, Dragon's Clutch, as research, and I use it as this comment's
worked example because a concrete system makes the classification questions
exact. The positions are my own analysis as a commenter, argued from the
statutory text and from that example.

1. Classify a staged program milestone by milestone, from the economic facts
   in force at the milestone being classified, through interpretive guidance
   within the framework of the 2012 Product Definitions Adopting
   Release.#note_ref(3)
2. On the example's terms, publication issues no claim and locks no
   collateral. Deposit is its first instrument-bearing milestone; later order
   interaction presents distinct venue and conduct facts. Other terms may
   produce other milestones.
3. Analyze formation from the rights and obligations created at each
   milestone, not from a universal proxy such as publication, signature, or
   funding.
4. Distinguish three questions this architecture makes easy to conflate: the
   gross classification of each issued claim; the holder's net economic
   exposure across claims; and the effect of a reversible complete-set
   operation. In the example, each claim has a contingent payoff while a
   complete set has a constant aggregate payout and may be recombined for
   collateral before resolution --- and that constancy holds identically for
   the smooth claim family, where it is a machine-checked theorem rather
   than a two-line arithmetic check.
5. In answer to Question 8, use complete-set decomposition as a diagnostic,
   not as an axiom that economic equivalence settles legal classification.
   For any proposed criterion, guidance should identify the unit classified,
   state whether gross rights or net exposure controls, and explain any
   change in result when the holder uses an economically reversible deposit
   or recombination operation.
6. Full prefunding, bounded loss under fully paid terms, and deterministic
   fail-closed settlement are risk facts, material where credit or
   customer-protection rules make them relevant. They are not classification
   exclusions and not proof that economic leverage is absent.
7. The instrument, the venue, the intermediary, and clearing and settlement
   are separate findings that arise at different milestones and can attach to
   different persons; "the software" is not a unit of classification.
...
transferability should carry under a particular statutory prong.
...
and include a deterministic rule for every edge case: a missing observation,
a malformed data page, a value exactly on a boundary.
...
slot], using valueless test tokens, fresh keys, and no customer anything.
The default build refuses every deposit until an authenticated data-source
release is compiled into it, and none is: the deposit instruction fails
closed. There is no mainnet deployment, no offer, and no live order from any
other person. Nothing in this comment requests permission to deploy
anything.
...
- *Claim families.* The simplest terms partition the price range into
  ...
  native generalization replaces bands with a smooth basis: each claim is
  one basis function of an open-clamped B-spline of degree one, two, or
  three over the frozen grid, and a position is a vector of integer
  coefficients over those claims. Settlement evaluates the basis at the
  observed value in exact rational arithmetic and converts to integer
  payouts by a canonical largest-remainder rule. Because the basis functions
  are nonnegative and sum to one everywhere on the grid --- the partition of
  unity, proved in a machine-checked model of the construction --- one claim
  of every basis function pays exactly the fixed denominator at any settled
  value: the same complete-set constancy as the five bands, now as a
  theorem. A payoff curve such as a capped call spread is expressed exactly
  by integer coefficients when it lies in the spline span, and with a
  certified error bound when it does not.
...
Every example here references objectively verifiable crypto-native facts:
ledger states, program events, prices, ranges, and path statistics --- none
...
kinds.#note_ref(1) A staged program separates into distinct dated events what
a bilateral confirmation fuses into one signing --- authorship, signature,
funding, interaction, resolution, and settlement can be days apart, with
different parties present and different rights enforceable at each --- so the
statutory question has a potentially different answer at each milestone, and
guidance that does not name the milestone it analyzes will be applied
inconsistently. Question 1 asks whether new interpretations are warranted
"[t]aking into account" the 2012 release;#note_ref(3) a milestone matrix is
an answer in exactly that form. The strongest counterargument is
gamesmanship: designers relabeling stages to defer the regulated moment. But
the matrix reads rights, obligations, collateral lock, issuance, transfer,
and exposure, which are facts; renaming a stage changes nothing unless the
facts change, and where they genuinely change, different treatment is
accuracy, not evasion.

== Position 2: publication alone is not the operative event here

At publication there are no transaction parties, no issued claims, and no
collateral lock. On those facts, treating publication as the
product-formation event would classify reusable text rather than an
...
transaction-linked compensation, or pre-authorized funding --- and those
surrounding facts require their own analysis under Position 7; publication
supplies no safe harbor for them.
...
its enumerated prongs.#note_ref(1) The example therefore asks at each
milestone what rights and obligations the terms have actually created.
...
instrument-bearing milestone. The conclusion is deliberately
...
terms therefore support three descriptions at once:

- *Gross instrument classification:* separately issued and transferable
  rights, each read from its own terms and the governing statutory
  criterion.
- *Net economic exposure:* a complete-set holder has a constant aggregate
  payout under the stipulated resolution rule; selling or buying a component
  introduces contingent net exposure.
- *Complete-set operation:* deposit and recombination transform collateral
  and the claim set at the stated one-for-one rate before resolution.

"Economically reversible" describes the stipulated protocol terms and payoff
arithmetic, not transaction fees, latency, implementation failure,
...
demanding diagnostic for any proposed answer. In a security-reference
hypothetical corresponding to the commodity example, deposit would create
several separately transferable cash-or-nothing claims on disjoint bands ---
or a smooth coefficient vector --- holding the complete set would produce a
...
measure net position, and legal form, transferability, issuance, or a status
predicate may matter. The diagnostic forces those choices into the open. For
each proposed Question 8 criterion, the Commissions should state:
...
types, not risk controls.#note_ref(1) Full prefunding reduces counterparty
credit exposure and, under these terms, prevents issuance above the pool's
stated payout capacity. It does not decide product category and does not
establish an absence of economic leverage. Deterministic, fail-closed
settlement terms describe how a system handles specified inputs and
failures; they do not establish oracle integrity, operational availability,
or legal compliance.

Formal verification, where it exists, is evidence for the named property
under the named model and assumptions --- nothing more. My Lean models
establish specified complete-set, solvency, guarded-transition, and B-spline
construction and quantization claims; one pinned Verus run checks an
internal-transfer arithmetic seam; no checked refinement connects the
complete Rust and SBF implementation to those theorems. Guidance should
credit demonstrated risk controls when a credit, custody, or
customer-protection rule makes them relevant, without converting scoped
proofs into whole-system assurance or classification exclusions.
...
derivatives-clearing-organization functions.#note_ref(2) In the example the
instrument question arises at funding and the venue question at match ---
different milestones, potentially different persons. Guidance should make
four findings, not one: the instrument (rights, contingency, reference,
payout, transferability); the venue (interaction, matching, interface
operation, control); the intermediary (solicitation, order handling,
discretion, compensation); and clearing and settlement (custody, novation or
its absence, netting, credit substitution, default handling). The same
discipline applies to software activity: publishing a general-purpose
language, authoring product-specific terms, operating an interface, holding
custody, and receiving transaction-linked compensation are different facts,
and treating them all as "code" is too coarse in both directions.

A recent comment on this docket illustrates the need to identify the role of
status predicates. FalconX Bravo, Inc. would classify a cash-settled
perpetual contract on a single security or narrow-based security index by
whether it is listed under the security-futures framework: listed, a
security futures product; offered bilaterally, over the counter, on a
non-U.S. venue, or through a decentralized finance protocol, a security-based
swap.#note_ref(7) That proposal makes listing status part of the category
test, not merely evidence about where trading occurs. This comment does not
resolve whether the governing security-futures provisions require that
result; it asks the Commissions to state expressly when listing or venue
status is a constituent statutory product fact and when it is a separate
conduct or facility fact. Reference, payout, contingency, listing, and venue
can all be objective facts; guidance should identify which legal element
each one satisfies instead of allowing one label to silently substitute for
another. Nothing here asks the proceeding to decide any facility's
registration status.
...
4. State expressly that full prefunding, bounded participant loss under
   fully paid terms, fail-closed settlement, and precisely scoped formal
...
The positions are my analysis as a commenter; none is a legal opinion, and
this comment does not classify Dragon's Clutch, its security-reference
variant, or any third-party product. The devnet deployment is research
operation with valueless test tokens, not an offer. The assumed
non-security-commodity reference is an analytic stipulation, not a
conclusion about any actual digital asset.
